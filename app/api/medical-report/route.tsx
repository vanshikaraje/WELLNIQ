...
//         { success: false, error: "No messages provided for report generation" },
//         { status: 400 }
//       );
//     }
...
//     const mockReport = {
//       patientSummary: "Patient presented with various health concerns during voice consultation.",
//       chiefComplaints: extractComplaints(messages),
//       symptoms: extractSymptoms(messages),
//       assessment: "Based on the conversation, further evaluation may be needed.",
//       recommendations: [
//         "Continue monitoring symptoms",
//         "Schedule follow-up appointment if symptoms persist",
//         "Maintain healthy lifestyle"
//       ],
//       followUp: "Follow up in 1-2 weeks or sooner if symptoms worsen",
//       medications: [],
//       riskFactors: [],
...
//     // ✅ PRINT ENTIRE REPORT TO CONSOLE
//     console.log("📝 Generated Medical Report:");
//     console.dir(mockReport, { depth: null });
...
//     if (text.includes('pain') || text.includes('hurt')) complaints.push('Pain-related symptoms');
//     if (text.includes('fever') || text.includes('temperature')) complaints.push('Fever');
//     if (text.includes('headache') || text.includes('head')) complaints.push('Headache');
//     if (text.includes('tired') || text.includes('fatigue')) complaints.push('Fatigue');
//   });

//   return complaints.length > 0 ? complaints : ['General health consultation'];
// }
...
//   return symptoms.length > 0 ? symptoms : ['Symptoms discussed during consultation'];
// }
// import { NextRequest, NextResponse } from "next/server";
...
//     // Try to save to database with timeout handling
//     let savedReport = null;
//     try {
//       console.log("🔄 Attempting to save to database...");
      
//       // Set a timeout for database operation
//       const dbPromise = db.insert(medicalReports).values(reportData).returning();
//       const timeoutPromise = new Promise((_, reject) => 
//         setTimeout(() => reject(new Error('Database timeout')), 5000)
//       );
      
//       [savedReport] = await Promise.race([dbPromise, timeoutPromise]) as any;
      
//       console.log("✅ Report successfully saved to database:");
//       console.dir(savedReport, { depth: null });
      
//     } catch (dbError) {
//       console.warn("⚠️ Database save failed, using local storage fallback:", dbError);
      
//       // Fallback: Save to localStorage (client-side will handle this)
//       savedReport = {
//         id: `local-${Date.now()}`,
//         ...reportData,
//         createdAt: new Date(),
...
//       console.log("💾 Using local storage fallback for report:");
//       console.dir(savedReport, { depth: null });
//     }
...
//       fallbackMode: !savedReport?.id?.includes('local') ? false : true
//     });
...
//     if (text.includes('headache') || text.includes('head')) complaints.push('Headache');
//     if (text.includes('tired') || text.includes('fatigue')) complaints.push('Fatigue');
//     if (text.includes('cough')) complaints.push('Cough');
//     if (text.includes('cold')) complaints.push('Cold symptoms');
//   });
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      report: savedReport,
      message: "Medical report generated successfully",
      fallbackMode: savedReport?.id?.includes('local') || false
    });
